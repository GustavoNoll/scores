...
describe('Field Score Evaluator', () => {
  const createMockFieldScoreRule = (data: Partial<FieldScoreRule>): FieldScoreRule => {
    return {
      ...
      goodThreshold: 0,
      criticalThreshold: 0,
      goodThresholdAdditional: null,
      criticalThresholdAdditional: null,
      functionType: 'linear',
      ...data,
    } as FieldScoreRule;
  };
  ...
  test('evaluateLinear with two good and two critical thresholds', () => {
    const rule = createMockFieldScoreRule({
      goodThreshold: 10,
      criticalThreshold: 5,
      goodThresholdAdditional: 15,
      criticalThresholdAdditional: 18,
      functionType: 'linear',
    });
  ...
  test('evaluateLinear with one good and one critical threshold', () => {
    ...
    expect(evaluateFieldScore(12, rule)).toBe(0.8);
    expect(evaluateFieldScore(15, rule)).toBe(0.5);
    expect(evaluateFieldScore(25, rule)).toBe(0);
  });
  ...
  test('evaluateExponential with two good and two critical thresholds', () => {
    const rule = createMockFieldScoreRule({
      ...
      criticalThreshold: 5,
      goodThresholdAdditional: 15,
      criticalThresholdAdditional: 20,
  ...
  test('evaluateFieldScore should return null for unsupported function types', () => {
    ...
    expect(evaluateFieldScore(15, rule)).toBeNull();
  });
});
